...
test('simple object is correctly verified', () => {
...
test('unordered keys are correctly verified', () => {
    const data = { "a": "x", "c": 1, "b": ["y", "z"] };
    const signature = getSignature(JSON.stringify(data, Object.keys(data).sort()));
    expect(validateSignature(data, signature)).toBe(true);
});

test('unordered keys in signature is not verified', () => {
    const data = { "a": "x", "c": 1, "b": ["y", "z"] };
    const signature = getSignature(JSON.stringify(data));
    expect(validateSignature(data, signature)).toBe(false);
});

test('modified data is not verified', () => {
    const data = { "a": "x", "c": 1, "b": ["y", "z"] };
    const signature = getSignature(JSON.stringify(data, Object.keys(data).sort()));
    data["a"] = "y";
    expect(validateSignature(data, signature)).toBe(false);
...
test('wrong hash-algorithm on signature is not verified', () => {
    const data = { "a": "x", "b": ["y", "z"], "c": 1 };
